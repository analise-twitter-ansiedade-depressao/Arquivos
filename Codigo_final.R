...
#	Contagem de tweets por polaridade e por dia
...
d_pol = stat2 %>%
  select(dia, polaridade) %>%
  group_by(dia, polaridade) %>%
  count(polaridade)
...
plot_str(d_pol)
create_report(d_pol)
plot_correlation(na.omit(d_pol), maxcat = 5L)
...
token_delim <- " \\t\\r\\n.!?,;\"()"
unitoken <- ngram_tokenize(texto,char = FALSE, ngmin=1, ngmax=1)
